...
# Timeseries plot
def timeseries_plot(selected_name, date_range):
...
def map_plot(selected_name, date_range):
    filtered_df = df[(df['name'].isin(list(selected_name))) & 
                     (df['datetime'] >= date_range[0]) & 
                     (df['datetime'] <= date_range[1])].sort_values("datetime")
    hover = HoverTool(tooltips=[("pm10", "@pm10"), ("date", "@datetime_str")])
    points = gv.Points(filtered_df, 
                       kdims=['longitude', 'latitude'], 
                       vdims=['datetime_str', 'pm10']).opts(
                                                            color_index='pm10', 
                                                            cmap='viridis', 
                                                            colorbar=True, 
                                                            width=600, 
...
@pn.depends(name_selector.param.value, date_range_slider.param.value)
def update_plots(selected_name, date_range):
    ts_plot_obj = timeseries_plot(selected_name, date_range)
    map_plot_obj = map_plot(selected_name, date_range)
    return pn.Row(pn.panel(ts_plot_obj), pn.panel(map_plot_obj))

dashboard = pn.Column(
    pn.Column(name_selector,
              date_range_slider,
              update_plots)
)
